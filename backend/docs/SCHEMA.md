...
- **EmailSuppression** (V47), email_hash (SHA-256 of lowercased address, unique), source (USER_REQUEST / BOUNCE / COMPLAINT), created_at. Holds only the CURRENT state: a row exists iff the address is suppressed right now. Suppressed hashes are skipped by all guest-facing sends (welcome, save-the-date, AND RSVP invites). Unsubscribe links and the delivery webhook write here; a couple resubscribing a guest deletes the row (`POST /api/v1/guests/couple/{coupleId}/{guestId}/resubscribe`, source-gated: USER_REQUEST/BOUNCE reversible, COMPLAINT refused).
- **EmailSubscriptionEvent** (V68), append-only audit trail (never deleted from) for CAN-SPAM defensibility, since resubscribe deletes the EmailSuppression row and would otherwise erase proof of the opt-out. Columns: email_hash (same SHA-256, no unique constraint, many rows per hash), action (SUPPRESSED / RESUBSCRIBED, DB CHECK), source (USER_REQUEST / BOUNCE / COMPLAINT / COUPLE_REQUEST), created_at. Written by EmailSuppressionAdapter inside the same transaction as every suppress/unsuppress. Index on (email_hash, created_at) backs the per-address timeline query.
