...
- **Email suppression model (two tables, The Knot/Zola scoping).** A guest is suppressed for a given couple's send when EITHER applies. Both write an audit row (below) in the same transaction.
  - **EmailSuppression** (V47) GLOBAL, address-level deliverability facts only: email_hash (SHA-256 of lowercased address, unique), source (BOUNCE / COMPLAINT, plus legacy USER_REQUEST from pre-V69 links), created_at. One row per address platform-wide; protects the shared altarwed.com sending reputation. Written by the Resend delivery webhook (permanent bounce / complaint). Never cleared by a couple/guest action except a legacy USER_REQUEST row, which a guest's RSVP clears.
  - **CoupleEmailOptOut** (V69) PER-COUPLE voluntary unsubscribes: (couple_id, email_hash) unique, created_at, FK to couples ON DELETE CASCADE. Unsubscribing from one couple's wedding mail does NOT silence another couple's. Written by the footer unsubscribe link (which now carries the sending couple, `?c={coupleId}`, in a couple-bound HMAC token). Resubscribe is recipient-initiated: a guest RSVPing on the couple's site clears their row here (`GuestService.submitRsvp` -> `resubscribeOnRsvp`). There is NO couple-side resubscribe endpoint.
- **EmailSubscriptionEvent** (V68), append-only audit trail (never deleted from) for CAN-SPAM defensibility, since the live state above is mutated/deleted over time. Columns: email_hash (same SHA-256, no unique constraint, many rows per hash), couple_id (nullable; null = global address-level event), action (SUPPRESSED / RESUBSCRIBED, DB CHECK), source (USER_REQUEST / BOUNCE / COMPLAINT / GUEST_RSVP), created_at. Written by both suppression adapters via SubscriptionAuditRecorder inside the same transaction as every state change. Index on (email_hash, created_at) backs the per-address timeline query.
...
- **SaveTheDateSend** (V86, issue #232), idempotency receipt for the save-the-date batch send: couple_id (FK couples ON DELETE CASCADE), idempotency_key (NVARCHAR(64), NOT NULL), queued_count, invalid_count, suppressed_count, created_at. Unique on (couple_id, idempotency_key). The client sends a per-attempt UUID; `GuestService.sendSaveDates` records one receipt before the async email fan-out, so a retry (lost HTTP response) or a concurrent duplicate replays the stored summary instead of re-emailing. Same concept as the print_orders idempotency model (V43) but a dedicated table (a save-the-date send has no order row to carry the key) and a different claim mechanism: the print-order orchestrator is non-transactional so its claim runs alone, whereas sendSaveDates is @Transactional (for the guest stamp), so the receipt insert uses REQUIRES_NEW + saveAndFlush to claim the key in its own transaction and surface a collision as a replay rather than a 500.
...
- **WeddingWebsite hero presentation** (V96, issue #360), two nullable columns on wedding_websites: `hero_overlay_darkness` INT (0-100 intensity of the dark scrim over the hero photo; null = default 70) and `hero_layout` NVARCHAR(20) (`full` = full-bleed cover crop, `framed` = contain the whole photo so portrait heroes are not cropped; null = full). Both are sanitised at the DTO boundary (`@Min(0)/@Max(100)` on darkness, `@Pattern(full|framed)` on layout) and re-clamped/allowlisted on the public render path (`frontend-public/src/lib/heroOverlay.ts`), so no untrusted value reaches the hero `<style>` sink.
...
- **PrintOrder test proof** (V98, issue #208): chk_print_order_type gains 'TEST_PROOF', a single self-addressed proof postcard the couple pays for (same $2 Stripe Checkout -> webhook -> async Lob path as a batch order; no free path) before ordering the full batch. print_order_recipients.guest_id becomes nullable: a TEST_PROOF recipient is the couple themselves, addressed from the order's persisted return_* block, not a guest row (FK to guests still validates non-null values).
